...
# Datos de SoyTechno (de la busqueda web)
soytechno = [
    {"store": "SoyTechno", "name": "Apple iPhone 16 Pro Max – 6.9\"", "price": "1979.00", "category": "Telefonos"},
    {"store": "SoyTechno", "name": "Apple iPhone 16 – 6.1\"", "price": "1139.00", "category": "Telefonos"},
    ...
    {"store": "SoyTechno", "name": "Monitor Smart Samsung M70C 27\" 4K UHD", "price": "519.00", "category": "Computacion"},
    {"store": "SoyTechno", "name": "All In One Lenovo IdeaCentre 23.8\" Ryzen 3", "price": "950.00", "category": "Computacion"},
]

# Datos adicionales de SoyTechno (encontrados en websearch)
soytechno_extra = [
    {"store": "SoyTechno", "name": "Apple iPhone 15 – 128GB / 1 SIM + eSIM", "price": "1139.00", "category": "Telefonos"},
    {"store": "SoyTechno", "name": "Telefono Celular Xiaomi", "price": "???", "category": "Telefonos"},
    {"store": "SoyTechno", "name": "Telefono Celular Samsung", "price": "???", "category": "Telefonos"},
]
...
# Comparar LatinBien con SoyTechno
for lb in latinbien:
    grupo = encontrar_coincidencias(lb, soytechno)
    if grupo:
...
<body>
    <div class="container">
        <header>
            <h1>Comparacion de Precios - Merida</h1>
            <p>LatinBien vs SoyTechno - Venezuela</p>
        </header>
        
        <div class="info-bar">
            <div><strong>Tiendas:</strong> LatinBien vs SoyTechno</div>
            <div><strong>Comparaciones:</strong> """ + str(len(comparaciones)) + """ productos</div>
        </div>
...
# Agregar tarjetas
for i, grupo in enumerate(comparaciones, 1):
    # Ordenar por precio
    ordenado = sorted(grupo, key=lambda x: float(x["price"]) if x["price"] != "???" else 999999)
    
    min_precio = float(ordenado[0]["price"]) if ordenado[0]["price"] != "???" else 0
    max_precio = float(ordenado[-1]["price"]) if ordenado[-1]["price"] != "???" else 0
    ahorro = max_precio - min_precio if min_precio > 0 else 0
    ahorro_pct = (ahorro / max_precio * 100) if max_precio > 0 else 0
    
    precios_html = ""
    for p in ordenado:
        precio = float(p["price"]) if p["price"] != "???" else 0
        clase = 'price-row'
        if p["price"] != "???" and precio == min_precio:
            clase += ' best-price'
        if p["price"] != "???" and precio == max_precio and len(ordenado) > 1:
            clase += ' worst-price'
